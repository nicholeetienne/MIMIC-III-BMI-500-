-- This code is mostly the same as AKI_stages_python. The difference is that the output are the patients before they reach an AKI stage between 1-3
-- Thus, the stage of '0'
-- This query is meant to check to see if the the patient had AKI according to KDIGO 
...
LEFT JOIN uo_stg uo
  ...
  WHERE
  -- Given that instruction state the AKI levels in the lecture slide range from 1-3 , the 0 stage is considered the stage before 
  uo.aki_stage_uo=0
  -- ordered based on the unique admission to an intensive care unit
